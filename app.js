...
function resizeCanvas() {
    ...
    canvas.height = screenEl.offsetHeight || 750;
}
resizeCanvas();
...
/* ============================================================
    !SCALE HELPER
    <>The canvas always draws in design-space (1280×750).
    <>getBoundingClientRect() returns scaled screen-space coords,
    <>so we divide by the current scale to convert back.
   ============================================================ */
function getCurrentScale() {
    const s = getComputedStyle(document.documentElement)
        .getPropertyValue('--mobile-scale').trim();
...
function polyCenter(points, divLeft, divTop, divW, divH) {
    ...
    points.forEach(([px, py]) => {
        cx += px / 100 * divW + divLeft;
        cy += py / 100 * divH + divTop;
    });
...
function getPolygonCanvasPoints(squareType) {
    ...
    const arenaX = (arenaRect.left - sRect.left) / scale;
    const arenaY = (arenaRect.top - sRect.top) / scale;
    const ringW = arenaRect.width / scale;
    const ringH = arenaRect.height / scale;
...
function getLaunchOrigin(player) {
    ...
    return {
        x: player === FIRE
            ? (rect.right - sRect.left) / scale - 60
            : (rect.left - sRect.left) / scale + 60,
        y: (rect.top - sRect.top) / scale + (rect.height / scale) * 0.46,
    };
}
    ...
    claimedTiles.forEach(tile => {
        ...
        if (isWinner) {
            ...
            const gc = tile.player === FIRE
                ? `rgba(255,110,0,${glowAlpha})` : `rgba(0,210,255,${glowAlpha})`;
            const gr = ctx.createRadialGradient(cx, cy, 0, cx, cy, glowR * 1.4);
            gr.addColorStop(0, gc); gr.addColorStop(1, 'rgba(0,0,0,0)');
            ctx.beginPath(); ctx.arc(cx, cy, glowR * 1.4, 0, Math.PI * 2);
            ctx.fillStyle = gr; ctx.fill();
        } else {
            const gc = tile.player === FIRE ? 'rgba(255,80,0,0.22)' : 'rgba(0,180,255,0.22)';
        ...
        let drawW, drawH;
        if (imgAspect > bboxAspect) { drawH = bboxH; drawW = bboxH * imgAspect; }
        else { drawW = bboxW; drawH = bboxW / imgAspect; }
...
function spawnImpact(squareType, player) {
    ...
    impacts.push({
        type: 'ring', x: pos.x, y: pos.y, r: 5,
        maxR: isfire ? 90 : 80, alpha: 1,
        color: isfire ? '255,140,0' : '80,210,255',
        life: 0, maxLife: 24
    });
    for (let i = 0; i < 40; i++) {
        ...
        impacts.push({
            type: 'dot', x: pos.x, y: pos.y,
            vx: Math.cos(angle) * speed, vy: Math.sin(angle) * speed,
            size: Math.random() * 5 + 2, alpha: 1,
            color: isfire
                ? (Math.random() > 0.5 ? '255,200,60' : '255,80,0')
                : (Math.random() > 0.5 ? '180,240,255' : '0,180,255'),
...
function burstParticles(winner) {
    const W = canvas.width, H = canvas.height;
    const cx = winner === FIRE ? W * 0.16 : W * 0.84;
    const cy = H * 0.45;
    const color = winner === FIRE ? '#ff6a00' : '#00c8ff';
    for (let i = 0; i < 80; i++) {
        burstList.push({
            x: cx, y: cy,
            vx: (Math.random() - 0.5) * 9, vy: (Math.random() - 1.5) * 7,
            size: Math.random() * 5 + 1, color, alpha: 1, life: 0,
...
function renderLoop() {
    ...
    impacts = impacts.filter(p => {
        ...
        if (p.type === 'ring') {
            ...
            ctx.strokeStyle = `rgba(${p.color},${p.alpha * 0.9})`;
            ctx.lineWidth = 4 - prog * 3; ctx.stroke();
            return p.life < p.maxLife;
        } else {
...
function updateTurnIndicator() {
    ...
    turnIndicator.textContent = isAI
        ? 'AI IS THINKING…'
        : `${currentPlayer === FIRE ? '🔥 FIRE' : '❄️ ICE'}'S TURN`;
...
function applyMove(squareType, player) {
    ...
    if (gameMode === '1P' && currentPlayer === ICE) {
        ...
        cells.forEach(c => {
            if (board[c.dataset.squareType] === null) c.style.pointerEvents = 'auto';
        });
    }
}
...
function checkWin(player) {
    for (const line of WIN_LINES)
        if (line.every(sq => board[sq] === player)) return line;
    return null;
}
...
//<> MINIMAX AI 
function doAIMove() {
    const best = minimax(board, ICE, 0);
    aiThinking = false;
...
function minimax(boardState, player, depth) {
    ...
    const moves = empty.map(sq => ({
        move: sq,
        score: minimax({ ...boardState, [sq]: player }, opp, depth + 1).score
    }));
    return player === ICE
...
scoreBoardBtn.addEventListener('click', () => {
    scoreFireEl.textContent = scores.fire;
    scoreIceEl.textContent = scores.ice;
...
function applyScale() {
    ...
    //<> PORTRAIT ON MOBILE → show rotate gate 
    if (isPortrait && isMobileDevice) {
        rotateOverlay.classList.add('portrait-active');
        screenEl.classList.add('portrait-hidden');
        cells.forEach(c => c.style.pointerEvents = 'none');
        return;
    }

    //<> LAPTOP/LANDSCAPE VIEW → hide rotate gate 
    rotateOverlay.classList.remove('portrait-active');
    screenEl.classList.remove('portrait-hidden');

    //<> Scale to fit the tighter axis, never upscale beyond 1 
    const scale = Math.min(vw / DESIGN_W, vh / DESIGN_H, 1);

    //<> Always set scale var + always add .scaled for centering 
    document.documentElement.style.setProperty('--mobile-scale', scale);
    screenEl.classList.add('scaled');

    //<> Center the scaled block in the viewport 
    const scaledW = DESIGN_W * scale;
    const scaledH = DESIGN_H * scale;
    screenEl.style.left = Math.max(0, (vw - scaledW) / 2) + 'px';
    screenEl.style.top = Math.max(0, (vh - scaledH) / 2) + 'px';

    //<> Restore pointer events for unclaimed cells 
    if (!gameOver && !aiThinking) {
        cells.forEach(c => {
            c.style.pointerEvents = board[c.dataset.squareType] === null ? 'auto' : 'none';
        });
    }
